...
$('#chooseCarFile').bind('change', function () {
    var filename = $("#chooseCarFile").val();
    if (/^\s*$/.test(filename)) {
        $("#chooseCarImage").removeClass('active');
        $("#noFile").text("No file chosen...");
    }
    else {
        $("#chooseCarImage").addClass('active');
        $("#noCarFile").text(filename.replace("C:\\fakepath\\", ""));
    }
});